...
export function ProductForm({ open, onOpenChange, product }: ProductFormProps) {
  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      setExistingImageUrls(product.images?.map((img: any) => img.url) || []);
    } else {
      reset({
        name: '',
        description: '',
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={categoryId} onValueChange={(val) => setValue('categoryId', val ?? '')}>
                <SelectTrigger>
                  <span className={selectedCategory?.name ? 'text-foreground' : 'text-muted-foreground'}>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-4 mt-2">
              {existingImageUrls.map((url, index) => (
                <div key={`existing-${url}-${index}`} className="relative w-24 h-24 rounded-lg overflow-hidden border">
                  <Image src={url} alt="Ảnh hiện có" fill className="object-cover" />
                  <button
                    type="button"
              ...
              {newImages.map((image, index) => (
                <div key={`new-${image.previewUrl}`} className="relative w-24 h-24 rounded-lg overflow-hidden border">
                  <Image src={image.previewUrl} alt="Ảnh mới" fill className="object-cover" unoptimized />
                  <button
                    type="button"

      ...
      <div className="bg-white border-b">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center text-sm text-gray-500">
            ...
            <span className="mx-2">/</span>
            <Link href={`/san-pham?category=${product.category?.slug}`} className="hover:text-green-600">
        ...
        <div className="bg-white rounded-3xl p-4 md:p-8 shadow-sm border border-gray-100 mb-8">
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
              ...
              {images.length > 1 && (
                <div className="flex gap-4 overflow-x-auto pb-2">
                  {images.map((img: any, idx: number) => (
                    <button 
                      key={img.id || idx}
                      className={`relative w-20 h-20 rounded-xl overflow-hidden border-2 shrink-0 ${mainImage === idx ? 'border-green-500' : 'border-transparent'}`}
            ...
            <div className="w-full lg:w-1/2 flex flex-col">
              {product.isFeatured && (
                <Badge className="bg-orange-500 hover:bg-orange-600 w-fit mb-4">Sản phẩm nổi bật</Badge>
              )}
              <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">{product.name}</h1>
              <div className="text-gray-500 mb-6 pb-6 border-b flex items-center gap-4">
                <span>Danh mục: <Link href={`/san-pham?category=${product.category?.slug}`} className="text-green-600 hover:underline">{product.category?.name}</Link></span>
                <span className="text-gray-300">|</span>
                <span>Tình trạng: <span className="text-green-600 font-medium">Sẵn sàng giao hàng</span></span>
              </div>
              ...
              <div className="bg-gray-50 p-6 rounded-2xl mb-8 border border-gray-100">
                <div className="flex flex-col sm:flex-row gap-4 items-center">
                  ...
                  <Button 
                    className="flex-1 h-12 rounded-full bg-green-600 hover:bg-green-700 text-base gap-2 w-full"
              ...
              {/* Guarantees */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-auto pt-6 border-t">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-green-50 text-green-600 rounded-full"><ShieldCheck size={20} /></div>
                  <span className="text-sm font-medium text-gray-700">100% Sạch & An toàn</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-green-50 text-green-600 rounded-full"><Truck size={20} /></div>
                  <span className="text-sm font-medium text-gray-700">Giao hàng 2 giờ</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-green-50 text-green-600 rounded-full"><RotateCcw size={20} /></div>
                  <span className="text-sm font-medium text-gray-700">Đổi trả 24h</span>
                </div>
              </div>
            </div>
          </div>
